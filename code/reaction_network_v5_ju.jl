...
if f_choice == "GC"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data("C:/Users/jd01548/OneDrive - University of Surrey/Code/tunneling_correction/julias/gc_rates.csv")
    k_f = rate_f[1] #change for neo/noneo
    k_r = rate_r[1] #change for neo/noneo
...
elseif f_choice == "mGC"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data("C:/Users/jd01548/OneDrive - University of Surrey/Code/tunneling_correction/julias/m_gc_rates.csv")
    k_f = rate_f_neo[1] #change for neo/noneo
    k_r = rate_r_neo[1] #change for neo/noneo
...
elseif f_choice == "GC_ext"     # 2 step reaction.
    dist, time_vals, rate_f, rate_r, rate2_f, rate2_r = load_reaction_data("C:/Users/jd01548/OneDrive - University of Surrey/Code/tunneling_correction/julias/gc_rates_extended_neo.csv")
    k_f = rate_f[1]
    k_r = rate_r[1]
    k2_ff = rate2_f[1]
    k2_rr = rate2_r[1]
    labs = LaTeXString[L"$G\mathrm{-}C$" L"$G\mathrm{+}C$" L"$G^\mathrm{-}\mathrm{-}C^\mathrm{+}$" L"$G^\mathrm{-}\mathrm{+}C^\mathrm{+}$" L"$G^\mathrm{*}\mathrm{-}C^\mathrm{*}$" L"$G^\mathrm{*}\mathrm{+}C^\mathrm{*}$"]
    network = rs_extended_m
    tc = 6.0    #8.0
    params = (:k1_f => k_f, :k1_r => k_r, :k_hel => k_helicase, :k2_f => k2_ff, :k2_r => k2_rr)
    u0 = [:R1 => 1.0, :R2 => 0.0, :R3 => 0.0, :P1 => 0.0, :P2 => 0.0, :P3 => 0.0]
end
