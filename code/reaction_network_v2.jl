...
rs_simple_m_t = @reaction_network begin     #this exp is a bad approx!! change for an interpolation!
    k_hel, R1 --> P1              # Helicase separation
...
if f_choice == "GC"
    #guess_decay = 1/0.03
    file_path = "C:/Users/jd01548/OneDrive - University of Surrey/Code/julias/gc_rates.csv"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data(file_path)
...
elseif f_choice == "mGC"
    #guess_decay = 1/0.02    
    file_path = "C:/Users/jd01548/OneDrive - University of Surrey/Code/julias/m_gc_rates.csv"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data(file_path)
    guess = @. rate_f[1] * exp(-guess_decay * time_vals)
    k_f = rate_f_neo[1]
    k_r = rate_r_neo[1]
...
if f_choice in ["GC"]
    ...
    plot_reactions(sol, labs; idx=[4, 5, 6], yscale=:identity, leg_loc=:right)  #4,5,6
    #writedlm( "GC_microkin.csv",  sol, ',')
    #CSV.write("GC_microkin.csv",  Tables.table(sol), writeheader=true)
end

if f_choice in ["mGC"]
    plot_reactions(sol, labs; name="plot_reaction_all_log", leg_loc=:right)
    plot_reactions(sol, labs; name="plot_reaction_all", yscale=:identity, leg_loc=:right)
    plot_reactions(sol, labs; idx=[3, 4], yscale=:identity, leg_loc=:right) #3,4    ############
    #writedlm( "mGC_microkin.csv",  sol, ',')
    #CSV.write("mGC_microkin.csv",  Tables.table(sol), writeheader=true)
end
