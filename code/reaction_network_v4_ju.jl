...
function save_solution(sol, labs, file_name)
    ...
    # Debug prints
    println("Number of labels: ", n)
    println("Number of time steps: ", length(sol.t))
    println("First few elements of sol.u: ", u[1:min(5, length(u))])

    # Convert time to SI
    time_sim = sol.t ./ au_time
    nt = length(time_sim)
    println("Converted time: ", time_sim[1:min(5, nt)])
...
if f_choice == "GC"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data("C:/Users/jd01548/OneDrive - University of Surrey/Code/julias/gc_rates.csv")
    k_f = rate_f_neo[1]
    k_r = rate_r_neo[1]
...
elseif f_choice == "mGC"
    dist, time_vals, rate_f, rate_r, rate_f_neo, rate_r_neo = load_reaction_data("C:/Users/jd01548/OneDrive - University of Surrey/Code/julias/m_gc_rates.csv")
    k_f = rate_f_neo[1]
    k_r = rate_r_neo[1]
...
if f_choice in ["GC"]
    ...
    save_solution(sol, labs, "GC_reaction_data.csv")
end
...
if f_choice in ["mGC"]
    ...
    save_solution(sol, labs, "mGC_reaction_data.csv")
end

#save_solution(sol, labs, "reaction_data.csv")
## Data saved to "Gonçalves De Abrantes, Juliana (PG/R - Chemistry & Chem Eng)"
